...
![Tests: 32 cases](https://img.shields.io/badge/tests-32%20cases-brightgreen)
...
- **Small, `React.memo`-wrapped components** named exactly to match the spec vocabulary: `ReceiptCard`, `SearchBar`, `FilterControls`, `ChapterCard`, `StoryTimeline`, `StoryMode`, `PatternInsights`, `ConnectionsGraph`, `PlacesFrequency`.
...
Use the left sidebar to search, filter by category, or set a date range.
...
│   │   ├── storyEngine.test.ts            ← 6 cases on chapter generation
│   │   ├── patternInsights.test.ts        ← 11 cases across 4 insight functions
│   │   ├── selectors.test.ts              ← 13 cases on filter / group / lookups
